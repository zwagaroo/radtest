...
class MyWindowPS(object):
    ...
    def initUI(self, MainWindow, identifier):
            ...
            for i in range (int(self.ps_num)):
                ...
                outputs_using = input("Enter which outputs you're using (seperate with space): ")

                outputs_using = outputs_using.split()
                    
                outputs_using = [int(value) for value in outputs_using]

                if(1 in outputs_using):
                ...
                if(1 in outputs_using and 2 in outputs_using):
                ...
                elif(1 in outputs_using):
                     self.dict_init_button["Init button PS{0}".format(i)].clicked.connect(lambda state, x=i: self.gpib(int(self.addr["Address{0}".format(x)]),int(x),self.voltage1["Voltage{0}".format(x)],self.current1["Current{0}".format(x)]))
                elif(2 in outputs_using):
                     self.dict_init_button["Init button PS{0}".format(i)].clicked.connect(lambda state, x=i: self.gpib(int(self.addr["Address{0}".format(x)]),int(x),None, None ,self.voltage2["Voltage{0}".format(x)],self.current2["Current{0}".format(x)]))
                ...
                if(1 in outputs_using and 2 in outputs_using):
                    ...
                    self.dict_on_button["PS{0} On button".format(i)].clicked.connect(lambda state, x=i: self.pwr_on(int(self.addr["Address{0}".format(x)]),int(x),self.voltage1["Voltage{0}".format(x)],self.current1["Current{0}".format(x)]))
                elif(2 in outputs_using):
                    self.dict_on_button["PS{0} On button".format(i)].clicked.connect(lambda state, x=i: self.pwr_on(int(self.addr["Address{0}".format(x)]),int(x),None, None ,self.voltage2["Voltage{0}".format(x)],self.current2["Current{0}".format(x)]))


                self.dict_PSOFF["PS{0} OFF".format(i)] = QtWidgets.QWidget(self.dict_PStabs["PS{0}".format(i)])
                self.dict_PSOFF["PS{0} OFF".format(i)].setGeometry(QtCore.QRect(150,155,130,150))
                ...
                if(1 in outputs_using and 2 in outputs_using):
                    ...
                    self.dict_off_button["PS{0} Off button".format(i)].clicked.connect(lambda state, x=i: self.pwr_off(int(self.addr["Address{0}".format(x)]),int(x),self.voltage1["Voltage{0}".format(x)],self.current1["Current{0}".format(x)]))
                elif(2 in outputs_using):
                    self.dict_off_button["PS{0} Off button".format(i)].clicked.connect(lambda state, x=i: self.pwr_off(int(self.addr["Address{0}".format(x)]),int(x),None, None ,self.voltage2["Voltage{0}".format(x)],self.current2["Current{0}".format(x)]))
                ...
                self.monitor_on_button["Monitor on{0}".format(i)].clicked.connect(lambda state, x=i: self.monitor_on(int(x), outputs_using))
                ...
                self.monitor_off_button["Monitor off{0}".format(i)].clicked.connect(lambda state, x=i: self.monitor_off(int(x), outputs_using))
                ...
                # Create an input field for setpoint voltage
                self.setpoint_voltage_input["Voltage Input{0}".format(i)] = QtWidgets.QLineEdit(self.dict_PStabs["PS{0}".format(i)])
                self.layout["Layout{0}".format(i)].addWidget(self.setpoint_voltage_input["Voltage Input{0}".format(i)])
                
                # Optionally, you can set a placeholder text
                self.setpoint_voltage_input["Voltage Input{0}".format(i)].setPlaceholderText("Enter Setpoint Voltage")
    ...
    def monitor_on(self,i, outputs_using):
        ...
        if(1 in outputs_using and 2 in outputs_using):
            ...
            self.thread_dict["Thread{0}".format(i)].started.connect(partial(self.worker_dict["Worker{0}".format(i)].run, int(self.addr["Address{0}".format(i)]),self.voltage1["Voltage{0}".format(i)],self.current1["Current{0}".format(i)]    ,None, None, int(i) ))
        elif(2 in outputs_using):
            self.thread_dict["Thread{0}".format(i)].started.connect(partial(self.worker_dict["Worker{0}".format(i)].run, int(self.addr["Address{0}".format(i)]),None, None, self.voltage2["Voltage{0}".format(i)],self.current2["Current{0}".format(i)], int(i) ))
        ...
        self.worker_dict["Worker{0}".format(i)].all_data.connect(partial(self.graph, int(i), outputs_using))
        self.worker_dict["Worker{0}".format(i)].GUI_PS.connect(partial(self.label_func, int(i), outputs_using))
        ...
        self.thread_dict["Thread{0}".format(i)].finished.connect(lambda: self.monitor_off(int (i), outputs_using))
    ...
    def label_func(self, i, outputs_using, par1, par2, par3, par4):
            print("Label GUI PS")
            if 1 in outputs_using:
    ...
    def graph(self, i, outputs_using, f1, f2, f3, f4, i1, obj1):
        print("Get PS Data")
        ...
        with open(self.PS_CSM_csvname, 'a') as csv_file:
            if(1 in outputs_using and 2 in outputs_using):
                ...
                fieldnames = ['DateTime','Time_S', 'Volt1_V PS'+str(i) , 'Curr1_A PS'+str(i)]
            elif(2 in outputs_using):
                fieldnames = ['DateTime','Time_S', 'Volt2_V PS'+str(i) , 'Curr2_A PS'+str(i)]
            ...
            if(1 in outputs_using and 2 in outputs_using):
                ...
                writer.writerow({'DateTime': d2, 'Time_S': str(i1), 'Volt1_V PS'+str(i): str(f1), 'Curr1_A PS'+str(i): str(f3)})
            elif(2 in outputs_using):
                writer.writerow({'DateTime': d2, 'Time_S': str(i1), 'Volt2_V PS'+str(i): str(f2), 'Curr2_A PS'+str(i): str(f4)})    
    ...
    def monitor_off(self,i, outputs_using):
            ...
            if(1 in outputs_using and 2 in outputs_using):
                ...
                self.IV_off["PS{0} IV off".format(i)]= IV_meas(int(self.addr["Address{0}".format(i)]),self.voltage1["Voltage{0}".format(i)],self.current1["Current{0}".format(i)])                        
            elif(2 in outputs_using):
                self.IV_off["PS{0} IV off".format(i)]= IV_meas(int(self.addr["Address{0}".format(i)]),None,None   ,self.voltage2["Voltage{0}".format(i)],self.current2["Current{0}".format(i)])                        
    ...
    def update(self, i ,outputs_using):
        if(1 in outputs_using):
